...
if (text.trim().startsWith('<')) { console.error('[SCI-TSS API] Respuesta HTML inesperada:', text.substring(0, 300));
    throw new Error( `El servidor devolvió HTML en lugar de JSON (HTTP ${response.status}). `
    + 'Revise los logs de Apache en XAMPP Control Panel.' ); } let result; try { result=JSON.parse(text); } catch
    (parseErr) { console.error('[SCI-TSS API] JSON inválido recibido:', text.substring(0, 300)); throw new
    Error('Respuesta del servidor inválida. Contacte al administrador.'); } if (!result.success) { throw new
    Error(result.message || 'Error en la petición al servidor.' ); } return result; } // ── GENERADOR DE AVATARES
    ───────────────────────────────────── function makeAvatar(name='?' , bg=null) { const
    words=String(name).trim().split(/\s+/); const initials=words.length>= 2
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 80 80">
        ...
        <text x="40" y="54" font-family="Arial,Helvetica,sans-serif" font-size="32" fill="white" text-anchor="middle"
    ...
    */
    ...
    if (!el) return;
    el.innerHTML = `<div class="alert alert-${type}" style="padding:10px 14px;border-radius:8px;font-size:.85rem;font-weight:500;
    ...
    btn.innerHTML = `<span
        style="display:inline-block;width:14px;height:14px;border:2px solid rgba(255,255,255,.35);border-top-color:#fff;border-radius:50%;animation:spin .6s linear infinite;vertical-align:middle;margin-right:6px;"></span>${text}`;
    } else {
    btn.disabled = false;
    ...
    { id: 1, username: 'admin', full_name: 'Administrador Principal', role: 'ADMIN', temporary_role: null, delegated_by:
    null, is_locked: false, failed_attempts: 0 },
    ]};
    }
